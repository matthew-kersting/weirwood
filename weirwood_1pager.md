...
The implementation has three major phases. Phases 1 and 2 are complete; Phase 3 is not yet started.

**Phase 1: Model loading and IR.** *(Complete.)* Trained XGBoost models are parsed from both JSON and Universal Binary JSON (UBJ) formats [7] into the `WeirwoodTree` intermediate representation. UBJ is XGBoost's default binary format since v1.6; it encodes the tree arrays (`split_indices`, `split_conditions`, `base_weights`, child pointers) in a compact binary layout. A `PlaintextEvaluator` runs standard f32 inference over the IR as a baseline reference. Full correctness is verified against XGBoost's Python output with a committed fixture (`trained_binary.ubj`, 100 trees, max_depth=8, 177 internal nodes, produced by `benchmarks/train_model.py`).

**Phase 2: FHE circuit evaluation.** *(Complete for tree evaluation; CKKS activation deferred.)* Key generation, fixed-point feature encryption (`i16`, scale factor 100), and decryption are implemented in `ClientContext`. The server-side FHE circuit is implemented in `FheEvaluator`: each internal node performs a TFHE programmable-bootstrapping comparison [6] that encodes the Heaviside step function as a lookup table, and an oblivious multiplexer (`if_then_else`) selects the left or right child score without revealing the branch taken. Leaf values are accumulated as `FheInt32` across all trees together with the base score. End-to-end FHE inference has been validated on the full `trained_binary.ubj` ensemble — 100 trees, max_depth=8, 177 internal nodes, requiring 177 bootstrapping operations per inference (~90 s single-threaded on CPU). The threshold comparison uses a ciphertext-vs-trivial-ciphertext path to avoid an overflow bug in tfhe-rs's scalar comparison accumulator. Sigmoid and softmax activations are currently applied client-side on the decrypted raw score rather than in CKKS; this avoids approximation error and is correct for the two-party protocol since the client decrypts before applying the activation. A CKKS activation layer would be needed only if the server must return an activated probability rather than a raw score. Parallelization across trees via Rayon is not yet implemented; it is the primary CPU-side optimization target for the next version.

**Phase 3: Transport and API.** *(Not started.)* Define wire types for encrypted inference requests and responses, serializable over gRPC or REST. The client generates keys, encrypts features, sends the ciphertext, and receives an encrypted prediction it can decrypt locally. The server holds only the evaluation key (no decryption capability).
