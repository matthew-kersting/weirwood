...
The comparison circuit itself introduces no approximation. TFHE programmable bootstrapping evaluates the Heaviside function exactly as a LUT, unlike BFV or BGV schemes which require polynomial approximation of comparisons \cite{TFHE-rs}. The oblivious evaluation strategy requires that both subtrees be computed at every internal node regardless of the comparison result, ensuring the execution trace is independent of the input and no branch information is leaked through timing or access patterns. The downside of this is that a PBS is computed for every possible node, whereas (in plaintext inference) we only need to visit relevant nodes.
...
weirwood (Rust, \textbf{FHE}) & 3.9 min         & 0.0042        & avg 10 runs, 525 PBS ops \\
\bottomrule
\end{tabular}
