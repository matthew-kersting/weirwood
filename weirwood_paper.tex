...
FHE \cite{gentry} supports arbitrary computation on ciphertexts, based on the hardness of Ring Learning With Errors (RLWE) \cite{regev}, believed hard for both classical and quantum computers. The central challenge is noise growth: additions accumulate error linearly and multiplications amplify it. Bootstrapping \cite{gentry} resets noise by homomorphically evaluating the decryption circuit, but is computationally expensive.
...
Rayon tree-level parallelism on 12 cores yields approximately a $9\times$ speedup over the expected serial time of $\approx 578$ s ($525 \text{ ops} \times 1.1$ s/op), rather than the theoretical $12\times$ maximum. The limited gain reflects the memory-bandwidth bottleneck of PBS operations: each bootstrapping call performs large Number Theoretic Transform (NTT) polynomial operations whose combined working set saturates L3 cache and DRAM bandwidth when executed in parallel \cite{TFHE-rs}. Each tree has on average 5.25 internal nodes (maximum path depth 6), so each Rayon task consists of up to 6 sequential PBS operations---sufficient work to amortize scheduling overhead, but insufficient to hide memory latency.

The primary remaining performance target is GPU acceleration. The \texttt{tfhe-rs} CUDA backend targets $\approx 1$ ms per PBS operation \cite{TFHE-rs}, which would reduce the 525-node model from $\approx 64$ s to under 1 s total inference latency.

\subsection{Security}

The security of \textit{weirwood} rests on the RLWE assumption at 128-bit security \cite{regev} using the default parameters from \texttt{tfhe-rs} \cite{TFHE-rs}. The best known attacks use lattice reduction via the Block Korkine--Zolotarev (BKZ) algorithm, which requires $2^{128}$ operations, making the scheme secure against both classical and quantum adversaries.
