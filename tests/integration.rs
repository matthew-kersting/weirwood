...
// Reference values are copied verbatim from XGBoost's Python output so they
// can be diffed against without mangling — keep the precision they ship with.
...
fn two_trees_raw_scores() {
    ...
    approx::assert_abs_diff_eq!(
        evaluator.predict(&loaded_tree, &[0.0, 0.0]),
        -0.5,
        epsilon = 1e-6
    );
    approx::assert_abs_diff_eq!(
        evaluator.predict(&loaded_tree, &[2.0, 3.0]),
        0.5,
        epsilon = 1e-6
    );
    approx::assert_abs_diff_eq!(
        evaluator.predict(&loaded_tree, &[0.0, 3.0]),
        -0.1,
        epsilon = 1e-6
    );
    approx::assert_abs_diff_eq!(
        evaluator.predict(&loaded_tree, &[2.0, 0.0]),
        0.1,
        epsilon = 1e-6
    );
}
...
fn two_trees_boundary_conditions() {
    ...
    approx::assert_abs_diff_eq!(
        evaluator.predict(&loaded_tree, &[1.5, 2.0]),
        -0.5,
        epsilon = 1e-6
    );
    // Just above both thresholds → both go right
    approx::assert_abs_diff_eq!(
        evaluator.predict(&loaded_tree, &[1.501, 2.001]),
        0.5,
        epsilon = 1e-5
    );
}
...
fn two_trees_predict_proba() {
    ...
    approx::assert_abs_diff_eq!(
        evaluator.predict_proba(&loaded_tree, &[0.0, 0.0]).unwrap(),
        0.37754066_f32,
        epsilon = 1e-5
    );
    // sigmoid(0.5) ≈ 0.62245934
    approx::assert_abs_diff_eq!(
        evaluator.predict_proba(&loaded_tree, &[2.0, 3.0]).unwrap(),
        0.62245934_f32,
        epsilon = 1e-5
    );
    // sigmoid(-0.1) ≈ 0.47502081
    approx::assert_abs_diff_eq!(
        evaluator.predict_proba(&loaded_tree, &[0.0, 3.0]).unwrap(),
        0.47502081_f32,
        epsilon = 1e-5
    );
    // sigmoid(0.1) ≈ 0.52497919
    approx::assert_abs_diff_eq!(
        evaluator.predict_proba(&loaded_tree, &[2.0, 0.0]).unwrap(),
        0.52497919_f32,
        epsilon = 1e-5
    );
}
...
fn regression_raw_scores() {
    ...
    approx::assert_abs_diff_eq!(evaluator.predict(&loaded_tree, &[0.0]), 0.5, epsilon = 1e-6);
    approx::assert_abs_diff_eq!(evaluator.predict(&loaded_tree, &[2.0]), 1.5, epsilon = 1e-6);
    approx::assert_abs_diff_eq!(evaluator.predict(&loaded_tree, &[1.5]), 0.5, epsilon = 1e-6); // boundary → left
    approx::assert_abs_diff_eq!(evaluator.predict(&loaded_tree, &[1.6]), 1.5, epsilon = 1e-6);
}
...
fn regression_predict_proba_is_identity() {
    ...
    let raw_score: f32 = evaluator.predict(&loaded_tree, &[2.0]);
    let predicted_proba: f32 = evaluator.predict_proba(&loaded_tree, &[2.0]).unwrap();
...
fn trained_json_matches_expected_outputs() {
    ...
    for (features, &expected_proba) in TRAINED_TEST_VECTORS.iter().zip(TRAINED_EXPECTED_PROBA) {
        let predicted_proba: f32 = evaluator.predict_proba(&loaded_tree, features).unwrap();
...
fn ubj_and_json_loaders_produce_identical_predictions() {
    ...
    for features in TRAINED_TEST_VECTORS {
        let json_predicted_proba: f32 = evaluator
            .predict_proba(&json_loaded_tree, features)
            .unwrap();
        let ubj_predicted_proba: f32 = evaluator.predict_proba(&ubj_loaded_tree, features).unwrap();
